...
export default function ShoppingCard({ card }: { card: Card }) {
  ...
  return (
    <div className="flex flex-row items-start mb-10 gap-8 h-[495px] border border-gray-300 rounded-2xl bg-white">
      <div className="w-[312px] h-[442px] mt-6 ml-6 shadow-[0_0_6.1px_5px_rgba(0,0,0,0.3)] bg-cover bg-center">
        <img src={card.image} alt={card.title} className="w-full h-full shadow-2xl" />
      </div>

      <div className="flex flex-col justify-between flex-1 h-full">
        <div className="flex flex-row justify-between items-start w-full">
          <div className="flex flex-col items-start w-80">
            <h1 className="text-2xl mt-4 font-semibold text-gray-900 font-['Nunito_Sans']">{card.title}</h1>

            <div className="flex flex-col mt-8 items-start gap-5 w-full">
              <Button
                onClick={() => navigate(`/personalize/${card.id}/step6`)}
              ...
              <div className="w-[111.53px] h-[158px] mt-5 bg-white rounded-[4px] shadow-[0_0_6.1px_5px_rgba(0,0,0,0.3)] flex items-center justify-center text-black text-[10px] font-medium">
                {card.message || "No message added"}
              </div>
            </div>
          </div>

          <div className="flex flex-row items-center gap-10 mr-6">
            <span className="text-xl font-normal underline text-gray-900 font-['Baloo']">
